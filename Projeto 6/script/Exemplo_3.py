...
def FiltroButterworth(D0, imgShape, n):
    ...
    for x in range(cols):
        for y in range(rows):
            base[y, x] = 1 - 1 / (1 + (distancia((y, x), center) / D0) ** (2 * n))
    return base
...
def FiltroGaussiano(D0, imgShape):
    ...
    for x in range(cols):
        for y in range(rows):
            base[y, x] = 1 - exp(((-distancia((y, x), center) ** 2) / (2 * (D0 ** 2))))
    return base
...
filtro_pa_ideal = FiltroIdeal(50, img.shape)
plt.subplot(152), plt.imshow(np.abs(filtro_pa_ideal), "gray"), plt.title("Filtro Ideal")

filtro_pa_Butterworth = FiltroButterworth(50, img.shape, 20)
plt.subplot(153), plt.imshow(np.abs(filtro_pa_Butterworth), "gray"), plt.title("Filtro Butterworth")

filtro_pa_Gaussiano = FiltroGaussiano(50, img.shape)
plt.subplot(154), plt.imshow(np.abs(filtro_pa_Gaussiano), "gray"), plt.title("Filtro Gaussiano")
...
centralizada_ideal = img_fft_centralizada * filtro_pa_ideal
...
centralizada_butterworth = img_fft_centralizada * filtro_pa_Butterworth
img_butterworth = np.fft.ifftshift(centralizada_butterworth)
...
centralizada_gaussiano = img_fft_centralizada * filtro_pa_Gaussiano
